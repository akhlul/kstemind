#' Stemmer Sastrawi - Porting of the most popular bahasa Indonesia Stemmer
#'
#' @docType class
#' @name stemmer_sastrawi
#' @export
stemmer_sastrawi <- R6::R6Class(
    classname    = "stemmer_sastrawi",
    lock_objects = FALSE,
	...
	public = list(
		...
		initialize = function(stem_dictionary = "kamus_sastrawi") {
			...
			private$visitors <- visitors_ecs$new(stem_dictionary = stem_dictionary, tables_visitors = "tabel_visitors_sastrawi")
		},
		...
		stem = function(word) {
		  ...
		  key <- list(word)
		  data <- loadCache(key, suffix=".sastrawi.Rcache")
		  if (!is.null(data)) {
		    return(data);
		  }
		  # 2. If not available, generate it.
		  data <- self$stem_word(word)
		  saveCache(data, key=key, suffix=".sastrawi.Rcache")
		  return(data)
		}
