#' Stemmer ECS - implementation of Enhanced CS Stemmer
#'
#' @docType class
#' @name stemmer_ecs
#' @export
stemmer_ecs <- R6::R6Class(
    classname    = "stemmer_ecs",
    lock_objects = FALSE,
    parent_env   = asNamespace('kstemind'),

  	#> TODO :
	#> []	encapsulate stem dictionary
	#> []   inside debug var for testing 'loop_pengembalian_akhiran'
	...
	public = list(
		...
		initialize = function(stem_dictionary = "kamus_sastrawi") {
			...
			private$visitors <- visitors_ecs$new(stem_dictionary = stem_dictionary)
		},
		...
		stem = function(word) {
		  ...
		  key <- list(word)
		  data <- loadCache(key, suffix=".ecs.Rcache")
		  if (!is.null(data)) {
		    return(data);
		  }
		  # 2. If not available, generate it.
		  data <- self$stem_word(word)
		  saveCache(data, key=key, suffix=".ecs.Rcache")
		  return(data)
		}		
	),
	private = list(
